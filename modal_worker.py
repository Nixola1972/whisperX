...
whisperx_image = (
    ...
    .apt_install(
        "git",
        "build-essential",      # C/C++ compilers (gcc, g++, make) for building PyAV
        "clang",                # Clang compiler (required by PyAV build system)
        ...
        "libavdevice-dev",      # FFmpeg device library (required by PyAV)
        "libavfilter-dev",      # FFmpeg filter library (required by PyAV)
        "libswscale-dev",
        "libswresample-dev"
    )
